    ...
    */
    void re_run(std::shared_ptr<PortalStmt> portal, QlManager* ql, Context *context) {
        switch(portal->tag) {
            case PORTAL_ONE_SELECT:
            {
                ql->re_run_select_from(std::move(portal->root), std::move(portal->sel_cols), context);
                break;
            }
    ...
    std::unique_ptr<AbstractExecutor> convert_plan_executor(std::shared_ptr<Plan> plan, Context *context)
    {
        if(auto x = std::dynamic_pointer_cast<ProjectionPlan>(plan)){
            ...
            if(x->tag == T_NestLoop) {
                return std::make_unique<BlockNestedLoopJoinExecutor>(std::move(left), 
                                std::move(right), std::move(x->conds_), context, x->sql_id_, x->plan_id_);
            }
            else {
                return std::make_unique<HashJoinExecutor>(std::move(left), 
                                std::move(right), std::move(x->conds_), context, x->sql_id_, x->plan_id_);
            }
