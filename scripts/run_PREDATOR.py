...
for query, parallel_factor, interval, cnt in param_sets:
    print(f"Running experiment with query={query}, parallel_factor={parallel_factor}, interval={interval}, cnt={cnt}")

    proxy_config_file_path = f"/root/SeamlessDB/src/config/proxy/proxy_config_{query}.json"
    compute_server_config_file_path = f"/root/SeamlessDB/src/config/PREDATOR/compute_server_config_p{parallel_factor}_{interval}.json"
    ...
    # 运行 active ro 并保存输出
    with open(f"{query}_active_{parallel_factor}_{interval}_{cnt}.txt", "w") as active_file:
        active_proc = subprocess.Popen(["./bin/rw_server", "active", "ro"], stdout=active_file)

    # 运行 backup ro 并保存输出
    with open(f"{query}_backup_{parallel_factor}_{interval}_{cnt}.txt", "w") as backup_file:
        backup_proc = subprocess.Popen(["./bin/rw_server", "backup", "ro"], stdout=backup_file)
    
    # sleep 15 秒
    time.sleep(15)

    output_file = f"{query}_proxy_{parallel_factor}_{interval}_{cnt}.txt"
