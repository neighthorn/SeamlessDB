...
struct PrimaryKey: public TreeNode {
    std::vector<std::shared_ptr<Col>> pkeys_;
    PrimaryKey(std::vector<std::shared_ptr<Col>> pkeys) : pkeys_(std::move(pkeys)) {}
};

struct CreateTable : public TreeNode {
    std::string tab_name;
    std::vector<std::shared_ptr<Field>> fields;
    std::shared_ptr<PrimaryKey> pkeys;

    CreateTable(std::string tab_name_, std::vector<std::shared_ptr<Field>> fields_, std::shared_ptr<PrimaryKey> pkeys_) :
            tab_name(std::move(tab_name_)), fields(std::move(fields_)), pkeys(std::move(pkeys_)) {}
};
...
struct SemValue {
    ...
    std::shared_ptr<PrimaryKey> sv_primarykey;
};
